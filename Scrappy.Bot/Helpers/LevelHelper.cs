...
public static class LevelHelper
{
    public static int CalculateLevelForXp(long totalXp)
    {
        ...
        var discriminant = 2500 - 20 * (100d - totalXp); // Calculate with double to prevent overflow
        var result = (-50 + Math.Sqrt(discriminant)) / 10;
    ...
    {
        // Get bounds
        var minXp = GetRequiredXpForLevel(currentLevel);
        var maxXp = GetRequiredXpForLevel(currentLevel + 1);

        // How much xp do we need to earn inside this level?
        var xpInLevel = currentXp - minXp;
        var xpRequiredForNextLevel = maxXp - minXp;

        var percentage = (double)xpInLevel / xpRequiredForNextLevel;
        percentage = Math.Clamp(percentage, 0.0, 1.0);

        // Build the bar
        var filledBlocks = (int)Math.Round(percentage * totalBlocks);
